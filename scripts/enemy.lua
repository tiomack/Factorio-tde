...
function spawn_defensive_units(nest_data, attacker)
  ...
  -- FIXED: Create unit_group using the new 2.0 API
  ...
  -- CRITICAL FIX: Use the new 2.0 commandable API and validate targets properly
  if units_spawned > 0 then
    -- Validate the attacker target before giving commands
    local command_target = nil
    
    if attacker and attacker.valid and attacker.health and attacker.health > 0 then
      -- Valid target with health
      command_target = attacker
    else
      -- Fallback to player base center if attacker is invalid or has no health
      command_target = find_player_base_center()
    end
    
    if command_target then
      -- FACTORIO 2.0 FIX: Use the commandable interface instead of direct set_command
      if unit_group.valid then
        if attacker and attacker.valid and attacker.health and attacker.health > 0 then
          ...
          -- Attack area around player base
          unit_group.set_command({
            type = defines.command.attack_area,
            destination = command_target,
            radius = 75,
            distraction = defines.distraction.by_anything
          })
        end
      ...
      log("TDE: No valid command target found for defensive units")
    end
    ...
    if spawner.unit_number then
      ...
      if nest_data and nest_data.is_defensive then
        ...
        if current_tick >= (nest_data.spawn_cooldown or 0) and math.random() < 0.3 then
          ...
          -- Pass nest_data properly to spawn_defensive_units
